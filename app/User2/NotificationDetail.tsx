...
const NotificationDetail = () => {
  ...
  const { notificationId, path, N_id } = route.params;
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      if (currentUser) {
        try {
          setLoading(true);
          const userRef = doc(db, "user2", email);
  ...
  const OnClickVerify = async (data: any) => {
    try {
      ...
      if (userName && data.approvalRef) {
        const EndorsedByDCE = userName;
      ...
      const user3Doc = await getDoc(user3Ref);

      const user3Token = user3Doc.exists()
        ? user3Doc.data().expoPushToken
        : null;

      if (user3Token) {
        await Promise.all([
          sendNotification(user3Token, {
        ...
        await addDoc(collection(db, "notification3"), {
          user: "user1",
          title: "Cannibalization Request Verified",
          body: "A new request has been verified. Please review it.",
          path,
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.reportContainer}>
        <Text style={styles.header}>Block A Details</Text>
        {notificationA ? (
          <>
        ...
        <Text style={styles.header}>Block B Details</Text>
        {notificationB ? (
          <>
            ...
            <Controller
              control={control}
              name="approvalRef"
              defaultValue="" // Default value for approvalRef
